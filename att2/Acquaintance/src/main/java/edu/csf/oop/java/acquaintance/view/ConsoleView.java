...
public class ConsoleView implements View {
    ...
    public ConsoleView() {
        try {
            profileService.restore("./config/last.json");
        } catch (Exception e) {
    ...
    public int exit() {
        try {
            System.out.println("Сохраняем последнее состояние..");
            profileService.save("./config/last.json");
    ...
    public void save() {
        ...
        try {
            profileService.save(filename + ".json");
            System.out.println("Успешно");
        } catch (Exception e) {
    ...
    public void restore() {
        scanner = new Scanner(System.in);
        System.out.print("Введите название файла из которого хотите восстановиться: ");
        String filename = scanner.nextLine();

        try {
            profileService.restore(filename + ".json");
        } catch (Exception e) {
    ...
    public void showMenu() {
        ...
        builder.append("10.Сохранить свой профиль\n");
        builder.append("11.Восстановить состояние своего профиля\n");
        builder.append("12.Выйти");
    ...
    public void refactorProfile() {
        ...
        try {
            for (int i = 0; i < 5; i++) {
                System.out.println("Введите номер поля для редактирования, а затем новое значение через пробел:");
                ...
                if (params.containsKey(Integer.parseInt(key))) {
                    System.out.println("Такое поле уже заполнено для изменения, введите другое");
                    i--;
                    continue;
                }

                params.put(Integer.parseInt(key), elements[1]);
            }
    ...
    private List<Profile> find() {
        ...
        return profileService.findProfiles(name, surname);
    }
    ...
    public void likePhoto() {
        ...
        try {
            ...
            profileService.like(photo, profile);
    ...
    public void commentPhoto() {
        ...
        try {
            ...
            profileService.comment(photo, comment, profile);
            System.out.println("Вы прокомментировали фотографию");
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
    }
    ...
    private Profile findProfile() throws Exception {
        ...
        System.out.print("Теперь введите его город: ");
